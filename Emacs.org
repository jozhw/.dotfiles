...
Year is the file boundary and month/day are headings inside it. Splitting
further --- a file per month --- was considered and rejected: it yields twelve
files a year, sixty after five years, each a few hundred lines, which is worse
for grepping and is exactly the kind of proliferation this setup exists to
avoid. Year is also the only unit anyone actually retires as a whole.

The *year comes from each entry's =CLOSED= date, not from today*. That is the
detail that makes the whole thing safe to neglect: a sweep run in January still
files December's work under =2026.org=, so the archive is accurate regardless of
when it is run. Computing the file per entry (rather than once at startup) also
avoids a session that stays open across New Year writing into the wrong year.

Note that =org-archive-location= is deliberately *not* set globally --- it is
bound only inside the command below. Setting it globally would redirect
archiving from *every* Org file, including denote notes, into the agenda
archive. Everything outside =todo.org= keeps Org's default of a
...
[[https://github.com/licht1stein/obsidian.el][obsidian.el]] brings an [[https://obsidian.md][Obsidian]] vault into Emacs: it treats a
...
It complements [[*denote.el][denote.el]] --- use denote for Org-based notes and obsidian.el

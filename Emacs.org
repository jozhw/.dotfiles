...
Since the compilation is sequential, you have to be careful in that =jw/leader-key-def= was defined in the *Key Bindings* section and it is used in the *UI Configuration* section under *Text Scaling* where =hydra= was used. This means that *Key Bindings* must be before the *Text Scaling* otherwise there will be a void error.

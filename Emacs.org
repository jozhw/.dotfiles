...
| =C-c n c= | =obsidian-capture=             | new named note in =notes/=            |
...
| =C-c n t= | =obsidian-insert-tag=          | insert a tag, completing on existing  |
...
obsidian.el applies a template in exactly one place: =obsidian-daily-note=
inserts =obsidian-daily-note-template= into a newly created daily note. Its
=obsidian-apply-template= is not an interactive command, so there is no built-in
way to use a template anywhere else. This adds one:
...
the one that pays here is the durable note, because it encodes the handoff rules
below --- claim first, sources kept:

#+begin_example
  # {{title}}

  <!-- one-sentence claim, stated flat -->
...
1. *Insert tags with =C-c n t=, never by typing them.* =obsidian-insert-tag=
   completes over tags already in the vault, so the path of least resistance is
   reuse. Typing =#= by hand is how a second spelling gets in.
...
The bank is meant to eventually be written up by someone else. A writer can turn
a rough correct idea into good prose but *cannot* reconstruct why something was
believed. So: *lead with the claim* (first line states the assertion, not the
background), *do not polish* (prose is the part being outsourced), and *keep the
sources* (links and citations cannot be recovered later).

One decision that is cheap now and near-impossible later: if a firm research
bank ever becomes real, make it a *separate vault* --- not for tidiness, but for
the moment other people get access.
